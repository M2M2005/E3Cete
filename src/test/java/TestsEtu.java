...
/**
 * etudiant.Tests unitaires à lancer pour vérifier que certains points de votre
 * application fonctionnent comme attendu.
 */
public class TestsEtu {
    ...
    public static void main(String[] args) {
        ...
        // runTest(TestsEtu::testTris, "testTris");
        // runTest(TestsEtu::testMelange, "testMelange");
    }
    ...
    private static Figure[] selectFigures() {
        return Arrays.copyOfRange(Figure.values(), 0, 3);
    }
    ...
    private static Texture[] selectTextures() {
        return Arrays.copyOfRange(Texture.values(), 0, 3);
    }
    ...
    private static Couleur[] selectCouleurs() {
        return Arrays.copyOfRange(Couleur.values(), 0, 3);
    }
    ...
    private static void compareCartes(Carte c1, Carte c2, int attendu) {
        ...
        if (attendu == -1 && compareResult > -1) {
            throw new TestException(String.format(
                    "Resultat négatif attendu en comparant %s et %s. Resultat obtenu : %s", c1, c2, compareResult));
        }
        if (attendu == 1 && compareResult < 0) {
            throw new TestException(
                    String.format("Resultat strictement positif attendu en comparant %s et %s. Resultat obtenu : %s",
                            c1, c2, compareResult));
        }
        if (attendu == 0 && compareResult != 0) {
            throw new TestException(String.format("Resultat nul attendu en comparant %s et %s. Resultat obtenu : %s",
                    c1, c2, compareResult));
        }
    }
    ...
    public static void testMelanger() {
        Paquet p = new Paquet(selectCouleurs(), 3, selectFigures(), selectTextures());
        p.melanger();
        Carte[] carteCourante = p.piocher(1);
        Carte[] prochaineCarte;
        while (!p.estVide()) {
            prochaineCarte = p.piocher(1);
            if (carteCourante[0].compareTo(prochaineCarte[0]) > 0) {
                return;
            }
        }
        throw new TestException(
                "Le paquet est complètement trié après un mélange. Cela est peu probable sur 81 cartes.");
    }
    ...
    public static void testTailleTable() {
        ...
        if (table.getTaille() != 9) {
            throw new TestException(
                    String.format("etudiant.Table 3x3 : taille 9 attendue (%s obtenu)", table.getTaille()));
        }
        table = new Table(2, 3);
        if (table.getTaille() != 6) {
            throw new TestException(
                    String.format("etudiant.Table 2x3 : taille 6 attendue (%s obtenu)", table.getTaille()));
        }
        table = new Table(4, 9);
        if (table.getTaille() != 36) {
            throw new TestException(
                    String.format("etudiant.Table 4x9 : taille 36 attendue (%s obtenu)", table.getTaille()));
        }
    }
    ...
    public static void testTris() {
        ...
        long tempsExec1 = Paquet.getTempsExecution(p::trierSelection);
        ...
        p.melanger();
        long tempsExec2 = Paquet.getTempsExecution(p::trierBulles);
        ...
        p.melanger();
        long tempsExec3 = Paquet.getTempsExecution(p::trierInsertion);
