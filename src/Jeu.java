...
public class Jeu {
            ...
            for (int j = i + 1; j < table.getTaille() - 1; j++) {
                for (int k = j + 1; k < table.getTaille(); k++) {
                    ...
                    if (estUnE3C(cartes123)) {
                        int[] numerosCartesE3C = { i, j, k };
                        return numerosCartesE3C;
                    }
                }
            }
        }
    ...
    public int[] selectionAleatoireDeCartesOrdinateur() {
        ...
        for (int i = 0; i < 3; i++) {
            selection[i] = Ut.randomMinMax(i * 3, (i * 3) + 2);
        }
        return selection;
    }
    ...
    public void jouerTourHumain() {
        Ut.afficherSL("Voici le score : " + this.score + "\nVoici la table : " + this.table);
        Ut.afficherSL("Choisissez 3 cartes (Format i,j) pour avoir un E3Cète.");
        ...
        if (estUnE3C(tabcartes)) {
            Ut.afficherSL("Vous avez trouvé un E3C.");
            this.score = this.score + 3;
        } else {
            Ut.afficherSL("Ce n'était pas un E3C.");
    ...
    public void jouerTourOrdinateur() {
        Ut.afficherSL("Voici le score : " + score + "\nVoici la table : " + table);
    ...
    public void jouer() {
        Ut.afficherSL("Démarrer une partie ? Veuillez saisir une option entre 1 et 3 :");
        Ut.afficherSL("(1) = Avec un joueur Humain | (2) = Avec un Ordinateur | (3) = Quitter le jeu");
        int choix = Ut.saisirEntier();
        if (choix == 1) {
            Ut.afficherSL("Partie lancée avec un joueur Humain");
            jouerHumain();
            resetJeu();
            jouer();
        }
        if (choix == 2) {
            Ut.afficherSL("Partie lancée avec un Ordinateur");
            jouerOrdinateur();
            resetJeu();
            jouer();
        }
        if (choix == 3) {
            ...
            Ut.afficherSL("Saisie non valide");
            jouer();
        }
    }
}
